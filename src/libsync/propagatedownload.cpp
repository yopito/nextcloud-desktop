...
QString OWNCLOUDSYNC_EXPORT createDownloadTmpFileName(const QString &previous)
{
    ...
    if (tmpPath.length() > 0) {
        return tmpPath + '/' + '.' + tmpFileName.left(spaceForFileName) + ".~" + (QString::number(uint((int) QRandomGenerator::global()->generate() % 0xFFFFFFFF), 16));
    } else {
        return '.' + tmpFileName.left(spaceForFileName) + ".~" + (QString::number(uint((int) QRandomGenerator::global()->generate() % 0xFFFFFFFF), 16));
    }
}
